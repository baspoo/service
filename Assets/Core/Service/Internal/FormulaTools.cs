...
public class FormulaToolsWindows : EditorWindow
{
	...
	Service.Formula.FormatSetup setup;
	...
	void OnGUI()
	{
		...
		if (toolsType == ToolsType.Convert)
		{
			...
			if (data != string.Empty)
			{
				...
				if (type == text)
					setup = (Service.Formula.FormatSetup)EditorGUILayout.EnumPopup(setup);
				if (GUILayout.Button("Convert")) 
				{
					...
					if (type == text) 
					{
						formulas.Add(Service.Formula.Setup(data, setup));
					}
					if (type == json)
					{
...
public class FormulaToolsService 
{
	...
	public class OnGUI
	{
		...
		public class GUIFormulaSerialize : Service.Formula
		{
			...
			{
				Service.Formula read = (string.IsNullOrEmpty(path)) ? f : f.Json.GetSubPath(path).SubFormula;
	...
	public class LocalStore
	{
		...
		//**[ Store ]**--------------------------------------------------------------
		static string keystore = "formula_store";
		static void addstore(string store)
		{
			string raw = PlayerPrefs.GetString(keystore);
			string datasave = Service.String.AddCommarString(store, raw, true);
			PlayerPrefs.SetString(keystore, datasave);
		}
		static void removestore(string store)
		{
			string raw = PlayerPrefs.GetString(keystore);
			string datasave = Service.String.DesCommarString(store, raw, true);
			PlayerPrefs.SetString(keystore, datasave);
		}
		public static List<string> GetStoreIDs()
		{
			string raw = PlayerPrefs.GetString(keystore);
			return (!string.IsNullOrEmpty(raw)) ? raw.Split(',').ToList<string>() : new List<string>();
		}
		...
		public static bool IsHave(string StoreName)
		{
			if (string.IsNullOrEmpty(StoreName))
				return false;
			string raw = PlayerPrefs.GetString(keystore);
			return Service.String.isCheckHaveinString(StoreName, raw);
		}
		//-------------------------------------------------------------------------
		...
		string keymember = "";
		string mailkey(string instanceID) { return keymember + "_" + instanceID; }
		void addmember(string member)
		{
			string raw = PlayerPrefs.GetString(keymember);
			string datasave = Service.String.AddCommarString(member, raw, true);
			PlayerPrefs.SetString(keymember, datasave);
		}
		void removemember(string member)
		{
			string raw = PlayerPrefs.GetString(keymember);
			string datasave = Service.String.DesCommarString(member, raw, true);
			PlayerPrefs.SetString(keymember, datasave);
		}
		bool ismember(string member)
		{
			if (string.IsNullOrEmpty(member))
				return false;
			string raw = PlayerPrefs.GetString(keymember);
			return Service.String.isCheckHaveinString(member, raw);
		}
		public void Save(string instanceID, Service.Formula formula)
		{
			if (!string.IsNullOrEmpty(instanceID))
			{
				PlayerPrefs.SetString(mailkey(instanceID), formula.PassToString());
		...
		public void Des(string instanceID)
		{
			if (!string.IsNullOrEmpty(instanceID))
			{
				PlayerPrefs.DeleteKey(mailkey(instanceID));
				removemember(instanceID);
			}
		}
		public void DesAll()
		{
			foreach (string ID in GetMemberIDs())
			{
				Des(ID);
			}
		}
		public Service.Formula GetFormula(string instanceID)
		{
			if (!ismember(instanceID))
				return null;
			string s_formula = PlayerPrefs.GetString(mailkey(instanceID));
			Service.Formula f = Service.Formula.Setup(s_formula);
			f.Nickname = instanceID;
			return f;
		}
		public List<Service.Formula> GetFormulas()
		{
			List<Service.Formula> output = new List<Service.Formula>();
			foreach (string ID in GetMemberIDs())
			{
				output.Add(GetFormula(ID));
			}
			return output;
		}
	...
	#region Json
	public class Json {
		Service.Formula formula;
		public Json(Service.Formula formula) {
			this.formula = formula;
		}
		public Service.Formula.FormulaData GetSubPath(string path) {
