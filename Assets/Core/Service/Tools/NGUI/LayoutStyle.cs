...
public class LayoutUI : Editor
{
    ...
    void StyleView(LayoutStyle.StyleData data)
    {
        ...
        if (m_tools.lastcurrent != null && m_tools.lastcurrent.ID == data.ID)
        {
            EditorGUIService.LableBlod(data.ID , Color.yellow);
        }
        else EditorGUIService.LableBlod(data.ID);
...
{
    public StylePackage AddStyle(string Name)
    {
        var style = new StylePackage() { StyleName = Name };
        StylePackages.Add(style);
        return style;
    }
    public StylePackage GetStyle(string Name)
    {
        return StylePackages.Find(x => x.StyleName == Name);
    }
    public void RemoveStyle(string Name)
    {
         StylePackages.RemoveAll(x => x.StyleName == Name);
    }
    public List<StylePackage> StylePackages = new List<StylePackage>();
}

public class StylePackage 
{
    public string StyleName;
    public LocalPackageContent AddContent(string Name , object date = null) {
        var content = new LocalPackageContent() { ObjectName = Name };
        Contents.Add(content);
        return content;
    }
    public LocalPackageContent GetContent(string Name)
    {
        return Contents.Find(x=>x.ObjectName == Name);
    }
    public List<LocalPackageContent> Contents = new List<LocalPackageContent>();
...
public class LayoutStyle : MonoBehaviour
{
    ...
    public class Tools
    {
        ...
        public static void RemoveStyle(LayoutStyle m, StyleData Style)
        {
            ...
            {
                obj.Catalog.RemoveStyle(Style.ID);
        ...
        static void save(LayoutStyle m, StyleData style)
        {
            ...
            foreach (var obj in m.LayoutDatas)
            {
                ...
                obj.Catalog.RemoveStyle(style.ID);
                var f_style = obj.Catalog.AddStyle(style.ID);
                //Service.Formula f_style = new Service.Formula();

                // Transform
                f_style.AddContent("activeSelf").Bol = obj.Transform.gameObject.activeSelf;
                f_style.AddContent("localPosition").vecter3 = obj.Transform.localPosition;
                f_style.AddContent("localRotation").quaternion = obj.Transform.localRotation;
                f_style.AddContent("localScale").vecter3 = obj.Transform.localScale;
                ...
                if (UILabel)
                {
                    f_style.AddContent("UILabel.text", 0).String = UILabel.text;
                    f_style.AddContent("UILabel.width", 0).Int = UILabel.width;
                    f_style.AddContent("UILabel.height", 0).Int = UILabel.height;
                    f_style.AddContent("UILabel.color", 0).color = UILabel.color;
                    f_style.AddContent("UILabel.overflowMethod", 0).Int = (int)UILabel.overflowMethod;
                    f_style.AddContent("UILabel.pivot", 0).Int = (int)UILabel.pivot;
                    f_style.AddContent("UILabel.depth", 0).Int = UILabel.depth;
                    f_style.AddContent("UILabel.fontSize", 0).Int = UILabel.fontSize;
                    f_style.AddContent("UILabel.fontStyle", 0).Int = (int)UILabel.fontStyle;
                    f_style.AddContent("UILabel.effectStyle", 0).Int = (int)UILabel.effectStyle;
                    f_style.AddContent("UILabel.effectColor", 0).color = UILabel.effectColor;
                    f_style.AddContent("UILabel.effectDistance", 0).vecter2 = UILabel.effectDistance;
                    f_style.AddContent("UILabel.spacingX", 0).Int = UILabel.spacingX;
                    f_style.AddContent("UILabel.spacingX", 0).Int = UILabel.spacingX;
                }
                ...
                if (UITexture)
                {
                    f_style.AddContent("UITexture.mainTexture", 0).texture = UITexture.mainTexture;
                    f_style.AddContent("UITexture.color", 0).color = UITexture.color;
                    f_style.AddContent("UITexture.pivot", 0).Int = (int)UITexture.pivot;
                    f_style.AddContent("UITexture.width", 0).Int = UITexture.width;
                    f_style.AddContent("UITexture.height", 0).Int = UITexture.height;
                    f_style.AddContent("UITexture.keepAspectRatio", 0).Int = (int)UITexture.keepAspectRatio;
                    f_style.AddContent("UITexture.depth", 0).Int = UITexture.depth;
                    f_style.AddContent("UITexture.border", 0).vecter4 = UITexture.border;
                    f_style.AddContent("UITexture.uvRect", 0).rect = UITexture.uvRect;
                }

                var UIButton = obj.Transform.GetComponent<UIButton>();
                if (UIButton)
                {
                    f_style.AddContent("UIButton.isEnabled", 0).Bol = UIButton.isEnabled;
                }

                var UIGrid = obj.Transform.GetComponent<UIGrid>();
                if (UIGrid)
                {
                    f_style.AddContent("UIGrid.arrangement", 0).Int = (int)UIGrid.arrangement;
                    f_style.AddContent("UIGrid.cellWidth", 0).Float = UIGrid.cellWidth;
                    f_style.AddContent("UIGrid.cellHeight", 0).Float = UIGrid.cellHeight;
                    f_style.AddContent("UIGrid.sorting", 0).Int = (int)UIGrid.sorting;
                    f_style.AddContent("UIGrid.pivot", 0).Int = (int)UIGrid.pivot;
                    f_style.AddContent("UIGrid.maxPerLine", 0).Int = (int)UIGrid.maxPerLine;
                    f_style.AddContent("UIGrid.animateSmoothly", 0).Bol = UIGrid.animateSmoothly;
                    f_style.AddContent("UIGrid.hideInactive", 0).Bol = UIGrid.hideInactive;
                    f_style.AddContent("UIGrid.keepWithinPanel", 0).Bol = UIGrid.keepWithinPanel;
                }

                var UIScrollView = obj.Transform.GetComponent<UIScrollView>();
                if (UIScrollView)
                {
                    f_style.AddContent("UIScrollView.contentPivot", 0).Int = (int)UIScrollView.contentPivot;
                    f_style.AddContent("UIScrollView.movement", 0).Int = (int)UIScrollView.movement;
                }

                var UIPanel = obj.Transform.GetComponent<UIPanel>();
                if (UIPanel)
                {
                    f_style.AddContent("UIPanel.depth", 0).Int = UIPanel.depth;
                    f_style.AddContent("UIPanel.clipping", 0).Int = (int)UIPanel.clipping;
                    f_style.AddContent("UIPanel.clipOffset", 0).vecter2 = UIPanel.clipOffset;
                    f_style.AddContent("UIPanel.clipSoftness", 0).vecter2 = UIPanel.clipSoftness;
                    f_style.AddContent("UIPanel.clipTexture", 0).texture2D = UIPanel.clipTexture;
                    f_style.AddContent("UIPanel.width", 0).Float = UIPanel.width;
                    f_style.AddContent("UIPanel.height", 0).Float = UIPanel.height;
                    f_style.AddContent("UIPanel.alpha", 0).Float = UIPanel.alpha;
                    f_style.AddContent("UIPanel.clipRange", 0).vecter4 = UIPanel.clipRange;
                }

                var TweenAlpha = obj.Transform.GetComponent<TweenAlpha>();
                if (TweenAlpha)
                {
                    f_style.AddContent("TweenAlpha.from", 0).Float = TweenAlpha.from;
                    f_style.AddContent("TweenAlpha.to", 0).Float = TweenAlpha.to;

                    f_style.AddContent("TweenAlpha.style", 0).Int = (int)TweenAlpha.style;
                    f_style.AddContent("TweenAlpha.duration", 0).Float = TweenAlpha.duration;
                    f_style.AddContent("TweenAlpha.delay", 0).Float = TweenAlpha.delay;
                }

                var TweenColor = obj.Transform.GetComponent<TweenColor>();
                if (TweenColor)
                {
                    f_style.AddContent("TweenColor.from", 0).color = TweenColor.from;
                    f_style.AddContent("TweenColor.to", 0).color = TweenColor.to;

                    f_style.AddContent("TweenColor.style", 0).Int = (int)TweenColor.style;
                    f_style.AddContent("TweenColor.duration", 0).Float = TweenColor.duration;
                    f_style.AddContent("TweenColor.delay", 0).Float = TweenColor.delay;
                }

                var TweenPosition = obj.Transform.GetComponent<TweenPosition>();
                if (TweenPosition)
                {
                    f_style.AddContent("TweenPosition.from", 0).vecter3 = TweenPosition.from;
                    f_style.AddContent("TweenPosition.to", 0).vecter3 = TweenPosition.to;

                    f_style.AddContent("TweenPosition.style", 0).Int = (int)TweenPosition.style;
                    f_style.AddContent("TweenPosition.duration", 0).Float = TweenPosition.duration;
                    f_style.AddContent("TweenPosition.delay", 0).Float = TweenPosition.delay;
                }

                var TweenScale = obj.Transform.GetComponent<TweenScale>();
                if (TweenScale)
                {
                    f_style.AddContent("TweenScale.from", 0).vecter3 = TweenScale.from;
                    f_style.AddContent("TweenScale.to", 0).vecter3 = TweenScale.to;

                    f_style.AddContent("TweenScale.style", 0).Int = (int)TweenScale.style;
                    f_style.AddContent("TweenScale.duration", 0).Float = TweenScale.duration;
                    f_style.AddContent("TweenScale.delay", 0).Float = TweenScale.delay;
                }

                var TweenRotation = obj.Transform.GetComponent<TweenRotation>();
                if (TweenRotation)
                {
                    f_style.AddContent("TweenRotation.from", 0).vecter3 = TweenRotation.from;
                    f_style.AddContent("TweenRotation.to", 0).vecter3 = TweenRotation.to;

                    f_style.AddContent("TweenRotation.style", 0).Int = (int)TweenRotation.style;
                    f_style.AddContent("TweenRotation.duration", 0).Float = TweenRotation.duration;
                    f_style.AddContent("TweenRotation.delay", 0).Float = TweenRotation.delay;
                }
        ...
        static void read(LayoutStyle m, string StyleID)
        {
            ...
            void push(string key, StylePackage package , System.Action<StylePackage.LocalPackageContent> action)
            {
                ...
                var data = package.GetContent(key);
                if (data!=null)
                {
                    action?.Invoke(data);
                }
            ...
            foreach (var obj in m.LayoutDatas)
            {
                ...
                {
                    var f_style = obj.Catalog.GetStyle(StyleID);
                    if (f_style != null)
                    {
